...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (add-to-list 'load-path "~/.emacs/elpa/org-20160711")
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/async-20160108.1249")
        (require 'async)
        )
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    (if (>= emacs-major-version 24)
        (progn
          (add-to-list 'load-path "~/.emacs.d/elpa/fullframe-20160209.2318")
          (require 'fullframe)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/smartparens-20160217.1102")
...
cannot live without it. Some help for Perl and even for org-mode.
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path
               "~/.emacs.d/elpa/yasnippet-20160131.948")
  (require 'yasnippet)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/flycheck-20160218.41")
        (require 'flycheck)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        ...
        (require 'guide-key)
        (setq guide-key/guide-key-sequence '("C-x" "C-x 4" "C-c" "C-h" "C-x l" "<ESC>" "<f9>"))
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elpa/avy-20160202.2357")
  (require 'avy)
...
#+BEGIN_SRC emacs-lisp
    (if (>= emacs-major-version 24)
        (progn
          (add-to-list 'load-path "~/.emacs.d/elpa/org-plus-contrib-20150831" t)
          (require 'org-mime)
          ))
...
A little macro to speed things up:

Bound to C-c J stores the org subtree as Jira markup language in clipboard.

#+BEGIN_SRC emacs-lisp
  (fset 'bba-org-export-jira-clipboard
     (lambda (&optional arg) "Keyboard macro." (interactive "p") (kmacro-exec-ring-item (quote ([24 110 115 escape 58 40 115 101 116 113 32 111 114 103 45 101 120 112 111 114 116 45 98 97 98 101 108 45 101 118 97 108 117 97 116 101 32 110 105 108 41 return 3 5 106 106 67108896 14 14 14 23 24 104 escape 119] 0 "%d")) arg)))
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key org-mode-map "\C-c\S-w" 'bba-copy-org-src-block)
#+END_SRC
...
| C-c F8       | workhours              |
| C-c F6       | work notes             |
| C-c Shift F6 | work notes archive     |
| C-c F9       | emacs cheat sheet      |
| C-c F10      | main emacs config file |
...
** [[http://emacs.stackexchange.com/questions/19037/org-babel-invoking-cmd-exe][windows stuff]]
[2016-09-07 Wed 12:10]

On windows we have to run cmdproxy.exe instead of sh.
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'ac-dictionary-directories "~/.emacs.d/elpa/auto-complete-20160107.8")
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/ggtags-20151214.1344" t)
        (require 'ggtags)
        (add-to-list 'load-path "~/.emacs.d/elpa/helm-gtags-20160202.503" t)
        (require 'helm-gtags)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path
                     "~/.emacs.d/elpa/helm-20160530.424")
        (require 'helm)
        (helm-mode 1)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/helm-descbinds-20160108.1947")
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elpa/key-chord-20151209.104")
...
#+BEGIN_SRC emacs-lisp
  ...
  (if (display-graphic-p)
      (progn
        (custom-set-faces
         '(org-block-begin-line ((t (:background "#29759c" :foreground "#84cbef"))) t)
